...
def threading_query(datasets, folder_name, BATCH_SIZE = 8, SLEEP_TIME = 1, query_function = get_image_response, 
    ...
    def target(prompt, num, base_num):
        ...
        img = query_function(prompt, model=model, size=size, quality=quality, timeout=timeout)
...
def generate_images(final_lyrics, folder_name="/root/project_output/gen_images", model="dall-e-3", size="1024x1024", quality="standard", sleep_time=1):
    # 图片生成
    ...
    # 多线程版本
    threading_query(final_lyrics, folder_name, query_function=get_image_response, model=model, size=size, quality=quality, SLEEP_TIME=sleep_time)
